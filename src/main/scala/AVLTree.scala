...
object AVLNode {
  ...
  def remove[A: Ordering](dataRemove: A)(tree: AVLTree[A]): AVLTree[A] = {
    val deletedImbalanced = removeUnbalanced(dataRemove)(tree)
  ...
  def removeUnbalanced[A: Ordering](dataRemove: A)(tree: AVLTree[A]): AVLTree[A] = {
    tree match {
      ...
      case AVLNode(data, left, right) =>
        ...
        order match {
          case -1 => AVLNode(data, removeUnbalanced(dataRemove)(left), right)
          case 1 => AVLNode(data, left, removeUnbalanced(dataRemove)(right))
          case 0 =>
